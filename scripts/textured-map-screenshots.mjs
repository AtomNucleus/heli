/**
 * Textured terrain screenshots for HELI STRIKE Poly Haven wiring.
 * Requires: npm run dev on http://127.0.0.1:5173, google-chrome, playwright-core.
 *
 * Outputs:
 *   heli-textured-map-title.png
 *   heli-textured-map-flight.png
 */
...
async function main() {
  ...
  // Wait for Poly Haven terrain textures to finish loading
  ...
  const texInfo = await page.evaluate(() => {
    const g = window.__game || window.__HELI_DEBUG;
    const mat = g.world.terrain.mesh.material;
    const mapImg = mat.map?.image;
    const isCanvas = typeof HTMLCanvasElement !== 'undefined' && mapImg instanceof HTMLCanvasElement;
    const shader = mat.userData?.shader;
    return {
      mapType: mat.map?.constructor?.name ?? null,
      mapIsCanvas: isCanvas,
      mapSrc: typeof mapImg?.src === 'string' ? mapImg.src : null,
      hasNormalMap: !!mat.normalMap,
      normalScale: mat.normalScale ? [mat.normalScale.x, mat.normalScale.y] : null,
      cacheKey: mat.customProgramCacheKey?.() ?? null,
      hasSandUniform: !!shader?.uniforms?.uSand?.value,
      sandIsCanvas:
        typeof HTMLCanvasElement !== 'undefined' &&
        shader?.uniforms?.uSand?.value?.image instanceof HTMLCanvasElement,
    };
  });
  console.log('terrainTextures', texInfo);
  ...
  await sleep(700);
  const titlePath = path.join(OUT, 'heli-textured-map-title.png');
  ...
  const flightPath = path.join(OUT, 'heli-textured-map-flight.png');
    ...
    JSON.stringify(
      {
        ...
        terrainTextures: texInfo,
        paths: [titlePath, flightPath],
      },
      null,
      2,
    ),
  ...
  if (texInfo.mapIsCanvas || texInfo.sandIsCanvas) {
    console.error('FAIL: still using canvas procedural maps');
    process.exitCode = 3;
  }
}
